...
class GKTPP_Options {
	...
	public function save_plugin_tabs() {
		if ( isset( $_POST['gktpp-settings-submit'] ) ) {
			...
			if ( ( '' !== $_POST['url'] ) && ( isset( $_POST['hint_type'] ) ) && ( isset( $_POST['gktpp_post_titles'] ) ) ) {
	...
	private function show_dnsprefetch_info() {
		...
		<p>As you can see, the typical way DNS lookups occur is when each resource is requested by the browser. By inserting the below DNS prefetch hints at the top of the page, the browser will know to perform the DNS resolution before it is asked:</p>

		<div class="gktpp-code-block">
			<script src="https://gist.github.com/sarcastasaur/9b71fa3be44258a9af670f799effaad6.js"></script>
		</div>

		<p>The waterfall chart below clearly shows when the DNS resolution is made <a href="https://output.jsbin.com/keninux/2">after taking advantage of DNS prefetching:</a></p>
		...
		<p>The DNS lookups occur before each resource is requested, preventing that step from slowing down resource delivery. This can improve the loading of each resource by hundreds of milliseconds or more, which can be particularly helpful for mobile users.</p>
	...
	private function show_prefetch_info() {
		...
		<p>Link prefetching utilizes browser idle time to download resources that the user might require in the immediate future. When idle, the browser begins silently downloading the specified documents and stores them in the browser cache. When that resource is requested, it can be served immediately, instead of downloading the entire resource from scratch. Prefetching will allow an entire resource to be loaded, instead of simply resolving the DNS as in DNS prefetching.</p>

		<p>Prefetching resources can be especially useful for <a href="https://developers.google.com/web/fundamentals/performance/critical-rendering-path/optimizing-critical-rendering-path">critical resources</a> that will be loaded later in the page, or resources behind a redirect.</p>

		<p>For example, if you have a CSS, JavaScript, or Google font that will be requested by a page a user is likely to click on, prefetching this file allows the browser to download the file ahead of time, improving the future page load time.</p>

		<p>The waterfall chart below indicates when the browser loads a large image that has been prefetched:</p>
		...
		<ul>
			<li><a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Link_prefetching_FAQ">Firefox: Link prefetching FAQ</a></li>
	...
	private function show_prerender_info() {
		...
		<p>The prerender resource hint should used to notify the browser that a user will <b>probably</b> visit a web page soon to allow the browser to load the entire page before the user has chosen to navigate there. When the visitor does click to navigate to the page, it will be <b>displayed immediately</b>.</p>

		<p>Prerendering a URL loads all of the related CSS, JavaScript, images, etc, that are required by the prefetched page. These files will also <b>be executed</b>. In some circumstances the request to prerender a page will be canceled by the user's browser for various reasons, such as limited bandwidth, or pop-up messages from the prerendered page.</p>

		<p>Since prerendering can be resource intentize, this should only be used if you are confident a particular page will be visited. Frequently this would be the most popular link visitors navigate towards, such as FAQ, information, or service pages.</p>

		<p><a href="https://output.jsbin.com/fenamaq">This demo website</a> has the URL "https://www.youtube.com" prerender hint enabled as below:<p>

		<div class="gktpp-code-block">
			<script src="https://gist.github.com/sarcastasaur/0b4161665b29262099f5fce295660708.js"></script>
		</div>

		<p>The waterfall chart of the website is below:</p>
		...
		<p>As you can see, after the given browser has finished loading the host URL's typical contents, the browser will begin loading the prerendered page in the background. So when a user clicks to navigate to YouTube, that website will be rendered instantly!</p>

		<p>For Chrome users, check out <a href="chrome://net-internals/#prerender">chrome://net-internals/#prerender</a> to see if a page has been successfully prerendered or not. If the prerender attempt was successful, you will see the following at the top of that page, along with past your past prerender history:</p>
		...
		<ul>
			<li><a href="https://www.stevesouders.com/blog/2013/11/07/prebrowsing/">Steve Souders "Prebrowsing"</a></li>
	...
	private function show_preconnect_info() {
		...
		<p>Typically the three steps involved in establishing a connection (DNS lookup, initial connection, and SSL negotiation) must be carried out when the resource is requested by the browser. Preconnecting allows these steps to be made proactively. This is similar to DNS prefetching, however preconnecting allows the initial connection and SSL negotiation to be resolved as well, instead of just the DNS lookup as is the case with DNS prefetching.</p>


		<p><a href="https://output.jsbin.com/dudeger">This example page</a> loads a Google Font, embedded YouTube iframe video, and a Google Analytics tracker in the typical manner, without taking advantage of the preconnect resource hint. A summary of the normal requests this demo website makes is shown in the chart below, courtesy of <a href="https://www.webpagetest.org">webpagetest.org</a></p>

		<h3 style="text-align: center;">Typical HTTPS Socket Negotiation Periods:</h3>
		...
		<p>Let's add the corresponding preconnect hints in the top of the web page and see what happens:</p>

		<div class="gktpp-code-block">
			<script src="https://gist.github.com/sarcastasaur/3329b4d00bd708015ad8b84a6d8fba9d.js"></script>
		</div>

		<h3 style="text-align: center;">HTTPS Socket Negotiation With Preconnect Enabled:</h3>
		...
		<p>As you can see, the three steps required to load resources from external domains occurs much earlier in the page request, reducing the need for these to be loaded earlier. The "crossorigin" attribute must be used when preconnecting to fonts, if this is left out only a DNS prefetch will be performed.</p>

		<p>Basically, preconnecting is DNS-prefetching, but also takes care of the initial connection and SSL negotiation, whereas DNS prefetching solely handles the DNS lookup.</p>


		<p>More information:</p>
		<ul>
			<li><a href="https://www.w3.org/TR/resource-hints/#dfn-preconnect">W3C: Preconnect</a></li>
	...
	private function show_preload_info() {
		...
		<p>This works in a similar manner as prefetching, however the preload hint is <b>mandatory, high priority, used for current navigation, and non-render blocking</b>. Whereas prefetching resources is optional (to the browser), low priority, and used to load resources a visitor is likely to require in the future.</p>

		<p>Let's see what the waterfall chart looks like after the following resource hints are placed into the top of a web page:</p>

		<div class="gktpp-code-block">
			<script src="https://gist.github.com/sarcastasaur/5683b903e195225ebb9e6379c80eba8c.js"></script>
		</div>

		<p>The waterfall chart below, courtesy of <a href="https://www.webpagetest.org">webpagetest.org</a>, shows when preloading resources are loaded by the browser:</p>
