...
//	public function __construct() {}
	...
	public function new_hint_controller( $raw_hint ) {
		...
		if ( is_array( $candidate_hint ) ) {
			$all_hints = \PPRH\Utils::get_pprh_hints(0);
			$dups = $this->handle_duplicate_hints( $all_hints, $candidate_hint );
	...
	public function handle_duplicate_hints( $all_hints, $candidate_hint ) {
		$this->duplicate_hints = $this->get_duplicate_hints( $all_hints, $candidate_hint );
		$duplicate_hints_exist = $this->duplicate_hints_exist( $this->duplicate_hints );

		if ( $duplicate_hints_exist ) {
			$resolved = $this->resolve_duplicate_hints( $candidate_hint );
	...
	public function resolve_duplicate_hints( $candidate_hint ) {

		if ( ! empty( $candidate_hint['post_id'] ) ) {
			return apply_filters( 'pprh_ch_resolve_duplicate_hints', $this->duplicate_hints, $candidate_hint );
