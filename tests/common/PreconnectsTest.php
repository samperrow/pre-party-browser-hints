...
class PreconnectsTest extends TestCase {
	...
	public function test_check_to_perform_reset() {
		$actual_1 = $this->preconnects->check_to_perform_reset( false, false, false );
		self::assertFalse( $actual_1 );

		$actual_2 = $this->preconnects->check_to_perform_reset( true, false, false );
		self::assertFalse( $actual_2 );

		$actual_3 = $this->preconnects->check_to_perform_reset( false, true, false );
		self::assertFalse( $actual_3 );

		$actual_4 = $this->preconnects->check_to_perform_reset( true, true, false );
		self::assertFalse( $actual_4 );

		$actual_5 = $this->preconnects->check_to_perform_reset( false, false, true );
		self::assertFalse( $actual_5 );

		$actual_6 = $this->preconnects->check_to_perform_reset( true, false, true );
		self::assertTrue( $actual_6 );

		$actual_7 = $this->preconnects->check_to_perform_reset( false, true, true );
		self::assertTrue( $actual_7 );

		$actual_8 = $this->preconnects->check_to_perform_reset( true, true, true );
		self::assertTrue( $actual_8 );
	}

	public function test_initialize() {
		$actual_1 = $this->preconnects->initialize( false, false, null );
		self::assertFalse( $actual_1 );

		$actual_2 = $this->preconnects->initialize( true, false, null );
		self::assertFalse( $actual_2 );

		$actual_3 = $this->preconnects->initialize( false, true, null );
		self::assertTrue( $actual_3 );

		$actual_4 = $this->preconnects->initialize( true, true, null );
	...
	public function test_load_ajax_callbacks() {
		...
		$callback = array( $this->preconnects, $ajax_cb );
	...
	public function test_post_domain_names() {
		...
		$actual_1 = $this->preconnects->post_domain_names( $pprh_data );
		self::assertFalse( $actual_1 );

		$actual_2 = $this->preconnects->post_domain_names( $pprh_data );
		self::assertFalse( $actual_2 );

		$pprh_data['hints'] = array( $hint_1, $hint_2 );
		$actual_3 = $this->preconnects->post_domain_names( $pprh_data );
		self::assertTrue( $actual_3 );

		$hint_3 = TestUtils::create_hint_array( '', 'preconnect' );
		$pprh_data['hints'] = array( $hint_1, $hint_3 );
		$actual_4 = $this->preconnects->post_domain_names( $pprh_data );
		self::assertFalse( $actual_4 );
		...
		$actual_5 = $this->preconnects->post_domain_names( $pprh_data_5 );
		self::assertFalse( $actual_5 );
	}
